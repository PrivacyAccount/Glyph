...
-- Funscript heatmap colors for seekbar (populated via script-message)
local heatmap_colors = {}
-- Optional timeline graph values (0..1) over seekbar
local timeline_line_points = {}
local timeline_line_enabled = false
local timeline_zoom_cache = { key = nil, sampled = nil }
-- Custom seekbar accent color (ASS format, e.g. &HF755A8&)
local custom_seekbar_color = nil
local glyph_script_sync_enabled = true
local glyph_script_available = false

local function get_theme_accent_color()
    return custom_seekbar_color or '&H42E39C&'
end
...
local language = {
	...
	['chs'] = {
		welcome = '{\\1c&H00\\bord0\\fs30\\fnå¾®è½¯é›…é»‘ light\\fscx125}MPV{\\fscx100} æ’­æ”¾å™¨',  -- this text appears when mpv starts
		off = 'å…³é—­',
		na = 'n/a',
		none = 'æ— ',
		video = 'è§†é¢‘',
	...
	['pl'] = {
	    welcome = '{\\fs24\\1c&H0&\\1c&HFFFFFF&}UpuÅ›Ä‡ plik lub Å‚Ä…cze URL do odtworzenia.',  -- this text appears when mpv starts
		off = 'WYÅ.',
		na = 'n/a',
		none = 'nic',
		video = 'Wideo',
		audio = 'ÅšcieÅ¼ka audio',
		subtitle = 'Napisy',
		available = 'DostÄ™pne ',
		track = ' ÅšcieÅ¼ki:',
...
function build_keyboard_controls()
    ...
    table.insert(bottom_button_line, 'tog_script')
    ...
    for n=1, #elements do
        ...
        if (element.type == 'slider') then
            ...
            if pos then
                ...
                -- When heatmap is active, only the seekbar is handled by render_heatmap.
                -- Other sliders (e.g. volume) must still draw their own fill+handle.
                local seekbar_heatmap_active = (#heatmap_colors > 0) and (element.name == 'seekbar')
                if not seekbar_heatmap_active then
        ...
        elseif (element.type == 'button') then
            ...
            -- è®¤ä¸º1ä¸ªä¸­æ–‡å­—ç¬¦çº¦ç­‰äºŽ1.5ä¸ªè‹±æ–‡å­—ç¬¦
...
function get_playlist()
    ...
    for i, v in ipairs(limlist) do
        ...
        message = string.format('%s %s %s\n', message,
            (v.current and '[x]' or '[ ]'), title)
    end
    return message
end

function get_next_playlist_item_title()
    ...
    local item = playlist[next_pos]
    if not item then return nil end
    local title = item.title
    if title == nil or title == '' then
        local _, filename = utils.split_path(item.filename or '')
        title = filename or item.filename
    end
    if title == nil or title == '' then return nil end
    return title
end

function toggle_loop_file()
    local loop_file = mp.get_property('loop-file', 'no')
    local enabled = loop_file ~= 'no'
    if enabled then
        mp.set_property('loop-file', 'no')
        show_message('Loop: OFF', 1.2)
    else
        mp.set_property('loop-file', 'inf')
        show_message('Loop: ON', 1.2)
    end
    request_tick()
end

local function set_glyph_script_sync_enabled(flag)
    local s = tostring(flag or ''):lower()
    glyph_script_sync_enabled = (s == '1' or s == 'true' or s == 'yes' or s == 'on')
    request_tick()
end

local function set_glyph_script_available(flag)
    local s = tostring(flag or ''):lower()
    glyph_script_available = (s == '1' or s == 'true' or s == 'yes' or s == 'on')
    request_init()
    request_tick()
end

local function notify_glyph_script_sync_state()
    mp.commandv('script-message', 'glyph-script-sync-state', glyph_script_sync_enabled and '1' or '0')
end

function toggle_glyph_script_sync()
    if not glyph_script_available then
        show_message('No funscript available', 1.2)
        request_tick()
        return
    end
    glyph_script_sync_enabled = not glyph_script_sync_enabled
    notify_glyph_script_sync_state()
    if glyph_script_sync_enabled then
        show_message('Script Sync: ON', 1.2)
    else
        show_message('Script Sync: OFF', 1.2)
    end
    request_tick()
end
...
function osc_init()
    ...
    ne.tooltipF = function ()
        local next_title = get_next_playlist_item_title()
        if next_title and next_title ~= '' then
            return 'Next: ' .. next_title
        end
        return 'Next'
    end
    ne.eventresponder['mbtn_left_up'] =
    ...
    ne.content = function ()
        ...
        if loop_file ~= 'no' then
            return '{\\fs28\\1c' .. get_theme_accent_color() .. '}⟲'
        end
        return '{\\fs28}⟲'
    end
    ne.tooltip_style = osc_styles.Tooltip
    ne.tooltipF = function ()
        local loop_file = mp.get_property('loop-file', 'no')
        if loop_file ~= 'no' then
            return 'Disable loop (L)'
        end
        return 'Enable loop (L)'
    end
    ...
    -- Glyph script sync toggle
    ne = new_element('tog_script', 'button')
    ne.enabled = glyph_script_available
    ne.content = function ()
        if not glyph_script_available then
            return '{\\1c&H999999&}FS'
        end
        if glyph_script_sync_enabled then
            return '{\\1c' .. get_theme_accent_color() .. '}FS'
        end
        return 'FS'
    end
    ne.tooltip_style = osc_styles.Tooltip
...
function render_heatmap(ass)
    if #heatmap_colors == 0 and not (timeline_line_enabled and #timeline_line_points > 1) then return end
    local se = state.slider_element
    if not se or not se.hitbox or not se.layout then return end

    local elem_geo = se.layout.geometry
    local slider_lo = se.layout.slider
    if not slider_lo or not se.slider then return end
    if not se.slider.min or not se.slider.max then return end

    local bar_min = se.slider.min.ele_pos
    local bar_max = se.slider.max.ele_pos
    if not bar_min or not bar_max then return end

    local num_segs = #heatmap_colors
    local bar_width = bar_max - bar_min
    local seg_w = (num_segs > 0) and (bar_width / num_segs) or 0

    -- Calculate absolute position of the seekbar
    local abs_x = elem_geo.x - elem_geo.w / 2
    local abs_y = elem_geo.y - elem_geo.h / 2
    local gap = slider_lo.gap or 0

    -- 1) Draw heatmap color segments
    if num_segs > 0 then
        for ci = 1, num_segs do
            local cx1 = abs_x + bar_min + (ci - 1) * seg_w
            local cx2 = abs_x + bar_min + ci * seg_w + 0.5
            local cy1 = abs_y + gap
            local cy2 = abs_y + elem_geo.h - gap

            ass:new_event()
            ass:pos(0, 0)
            ass:an(7)
            ass:append('{\\bord0\\shad0\\1c' .. heatmap_colors[ci] .. '}')
            ass_append_alpha(ass, se.layout.alpha, 0)
            ass:draw_start()
            ass:rect_cw(cx1, cy1, cx2, cy2)
            ass:draw_stop()
        end
    end

    -- 1b) Optional zoomed timeline graph only on hover (performance friendly)
    if timeline_line_enabled and #timeline_line_points > 1 and mouse_hit(se) then
        local hover_pos = get_slider_value(se)
        if hover_pos == nil then hover_pos = se.slider.posF() end
        if hover_pos ~= nil then
            local center_t = math.max(0, math.min(1, hover_pos / 100))
            -- Adaptive zoom window by absolute playback time:
            -- fixed-percent windows feel too zoomed-in on short videos and too zoomed-out on long ones.
            local duration_sec = mp.get_property_number("duration", 0) or 0
            local target_window_sec = 30
            local span_t = 0.02
            if duration_sec > 0 then
                span_t = target_window_sec / duration_sec
            end
            -- Clamp to sensible bounds (0.6% .. 25% of full timeline).
            if span_t < 0.006 then span_t = 0.006 end
            if span_t > 0.25 then span_t = 0.25 end
            local t_min = math.max(0, center_t - span_t / 2)
            local t_max = math.min(1, center_t + span_t / 2)
            if (t_max - t_min) < 0.001 then
                t_min, t_max = 0, 1
            end

            local panel_w = math.min(360, bar_width * 0.6)
            local panel_h = 54
            local cursor_x = abs_x + bar_min + (bar_width * center_t)
            local panel_x1 = math.max(abs_x + bar_min, math.min(cursor_x - panel_w / 2, abs_x + bar_max - panel_w))
            local panel_x2 = panel_x1 + panel_w
            local panel_y2 = abs_y - 6
            local panel_y1 = panel_y2 - panel_h

            ass:new_event()
            ass:pos(0, 0)
            ass:an(7)
            ass:append('{\\bord0\\shad0\\1c&H000000&}')
            ass_append_alpha(ass, se.layout.alpha, 65)
            ass:draw_start()
            ass:rect_cw(panel_x1, panel_y1, panel_x2, panel_y2)
            ass:draw_stop()

            -- Center marker line (cursor reference inside the hover panel).
            local center_x = panel_x1 + panel_w / 2
            ass:new_event()
            ass:pos(0, 0)
            ass:an(7)
            ass:append('{\\bord0\\shad0\\1c&HFFFFFF&}')
            ass_append_alpha(ass, se.layout.alpha, 35)
            ass:draw_start()
            ass:rect_cw(center_x - 0.8, panel_y1 + 1, center_x + 0.8, panel_y2 - 1)
            ass:draw_stop()

            local panel_pad = 2
            local inner_x1 = panel_x1 + panel_pad
            local inner_x2 = panel_x2 - panel_pad
            local inner_y1 = panel_y1 + panel_pad
            local inner_y2 = panel_y2 - panel_pad
            local inner_w = math.max(1, inner_x2 - inner_x1)
            local inner_h = math.max(1, inner_y2 - inner_y1)
            local function clamp(v, lo, hi)
                if v < lo then return lo end
                if v > hi then return hi end
                return v
            end
            local function map_x(tt)
                local x = inner_x1 + ((tt - t_min) / math.max(0.0001, (t_max - t_min))) * inner_w
                return clamp(x, inner_x1, inner_x2)
            end
            local function map_y(vv)
                local y = inner_y2 - (vv * inner_h)
                return clamp(y, inner_y1, inner_y2)
            end

            -- Cache downsampled points for nearby hover positions to reduce per-frame work.
            local cache_key = string.format("%.2f:%.2f:%d", t_min, t_max, #timeline_line_points)
            if timeline_zoom_cache.key ~= cache_key then
                local filtered = {}
                for i = 1, #timeline_line_points do
                    local p = timeline_line_points[i]
                    local t = tonumber(p.t)
                    local v = tonumber(p.v)
                    local pad_t = span_t * 0.18
                    if t and v and t >= (t_min - pad_t) and t <= (t_max + pad_t) then
                        table.insert(filtered, { t = t, v = math.max(0, math.min(1, v)) })
                    end
                end

                local max_points = 130
                if #filtered > max_points then
                    -- Preserve local movement by sampling per time-bucket with extremes.
                    local bucket_count = math.max(24, math.floor(max_points / 3))
                    local t_span = math.max(0.0001, (t_max - t_min))
                    local buckets = {}
                    for i = 1, #filtered do
                        local p = filtered[i]
                        local norm = (p.t - t_min) / t_span
                        local bi = math.floor(norm * (bucket_count - 1)) + 1
                        if bi < 1 then bi = 1 end
                        if bi > bucket_count then bi = bucket_count end
                        local b = buckets[bi]
                        if not b then
                            buckets[bi] = { first = p, last = p, min = p, max = p }
                        else
                            b.last = p
                            if p.v < b.min.v then b.min = p end
                            if p.v > b.max.v then b.max = p end
                        end
                    end

                    local sampled = {}
                    local function push_if_new(pt)
                        if not pt then return end
                        local prev = sampled[#sampled]
                        if not prev or prev.t ~= pt.t or prev.v ~= pt.v then
                            table.insert(sampled, pt)
                        end
                    end

                    for bi = 1, bucket_count do
                        local b = buckets[bi]
                        if b then
                            local pack = { b.first, b.min, b.max, b.last }
                            table.sort(pack, function(a, b2) return a.t < b2.t end)
                            for pi = 1, #pack do
                                push_if_new(pack[pi])
                            end
                        end
                ...
                timeline_zoom_cache.key = cache_key
                timeline_zoom_cache.sampled = filtered
            end

            local filtered = timeline_zoom_cache.sampled or {}
            if #filtered > 1 then
                local points = {}
                for i = 1, #filtered do
                    local p = filtered[i]
                    points[i] = { x = map_x(p.t), y = map_y(p.v), v = p.v }
                end

                -- Draw a continuous connector line between points.
                ass:new_event()
                ass:pos(0, 0)
                ass:an(7)
                ass:append('{\\bord0\\shad0\\1c&HA8EFA0&}')
                ass_append_alpha(ass, se.layout.alpha, 0)
                ass:draw_start()
                for i = 2, #points do
                    local a = points[i - 1]
                    local b = points[i]
                    local steps = math.max(1, math.floor(math.max(math.abs(b.x - a.x), math.abs(b.y - a.y)) / 2.6))
                    for s = 0, steps do
                        local tt = s / steps
                        local x = a.x + (b.x - a.x) * tt
                        local y = a.y + (b.y - a.y) * tt
                        ass:rect_cw(x - 0.55, y - 0.55, x + 0.55, y + 0.55)
                    end
                end
                ass:draw_stop()

                -- Endpoints in red for readability.
                ass:new_event()
                ass:pos(0, 0)
                ass:an(7)
                ass:append('{\\bord0\\shad0\\1c&H3030FF&}')
                ass_append_alpha(ass, se.layout.alpha, 0)
                ass:draw_start()
                for i = 1, #points do
                    local p = points[i]
                    ass:rect_cw(p.x - 1.0, p.y - 1.0, p.x + 1.0, p.y + 1.0)
                end
                ass:draw_stop()
            end
        end
    ...
    -- 2) Redraw progress fill on top of heatmap (using absolute coords)
    local pos = se.slider.posF()
    if pos then
        local xp = get_slider_ele_pos_for(se, pos)
        local rh = user_opts.seekbarhandlesize * elem_geo.h / 2

        -- Progress fill bar (from left edge to current position)
        ass:new_event()
        ass:pos(0, 0)
        ass:an(7)
        ass:append(osc_styles.SeekbarFg)
        ass_append_alpha(ass, se.layout.alpha, 0)
        ass:draw_start()
        ass:rect_cw(abs_x, abs_y + gap, abs_x + xp, abs_y + elem_geo.h - gap)
        ass:draw_stop()
        ...
        ass_append_alpha(ass, se.layout.alpha, 0)
        ass:draw_start()
        ass_draw_cir_cw(ass, abs_x + xp, abs_y + elem_geo.h / 2, rh)
        ass:draw_stop()
    end
...
function render()
    ...
    if state.osc_visible then
        render_elements(ass)
        -- Draw heatmap on top of seekbar background, then redraw dot/fill on top
        render_heatmap(ass)
    end
...
mp.add_key_binding('l', 'glyph-toggle-loop-file', function()
    toggle_loop_file()
end)
mp.add_key_binding('k', 'glyph-toggle-script-sync', function()
    toggle_glyph_script_sync()
end)
mp.register_script_message('glyph-set-script-sync', function(flag)
    set_glyph_script_sync_enabled(flag)
end)
mp.register_script_message('glyph-set-script-available', function(flag)
    set_glyph_script_available(flag)
end)
...
-- Funscript heatmap: receive CSV of ASS color codes (e.g. "&H342A24&,&H4CB05C&,...")
mp.register_script_message("set-heatmap-colors", function(csv)
    heatmap_colors = {}
    if csv and csv ~= "" then
        for color in csv:gmatch("([^,]+)") do
            table.insert(heatmap_colors, color)
        end
        msg.info("Loaded " .. #heatmap_colors .. " heatmap segments")
    end
    request_tick()
end)

mp.register_script_message("clear-heatmap-colors", function()
    heatmap_colors = {}
    request_tick()
end)

mp.register_script_message("set-timeline-line-enabled", function(flag)
    timeline_line_enabled = (tostring(flag) == '1' or tostring(flag) == 'true')
    request_init()
    request_tick()
end)

mp.register_script_message("set-timeline-line-data", function(csv)
    timeline_line_points = {}
    timeline_zoom_cache = { key = nil, sampled = nil }
    if csv and csv ~= "" then
        for value in csv:gmatch("([^,]+)") do
            local t_str, v_str = value:match("^%s*([%+%-]?[%d%.]+):([%+%-]?[%d%.]+)%s*$")
            if t_str and v_str then
                ...
                if t and v then
                    ...
                    if v < 0 then v = 0 end
                    if v > 1 then v = 1 end
                    table.insert(timeline_line_points, { t = t, v = v })
                end
            else
                -- Backward-compatible fallback: plain values, evenly distributed
                local n = tonumber(value)
                if n then
                    if n < 0 then n = 0 end
                    if n > 1 then n = 1 end
                    table.insert(timeline_line_points, { t = nil, v = n })
                end
            end
        end
    end
...
mp.register_script_message("clear-timeline-line-data", function()
    timeline_line_points = {}
    timeline_zoom_cache = { key = nil, sampled = nil }
    request_tick()
end)

-- Funscript heatmap peaks: receive CSV of percent positions (e.g. "12.5,34.0,78.2")
-- Seekbar accent color: receive ASS color from app theme
mp.register_script_message("set-seekbar-color", function(ass_color)
    if ass_color and ass_color ~= "" then
        custom_seekbar_color = ass_color
        osc_styles.SeekbarFg = '{\\blur1\\bord1\\1c' .. ass_color .. '}'
        msg.info("Seekbar color set to " .. ass_color)
        request_tick()
    end
end)
